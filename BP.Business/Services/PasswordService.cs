...
namespace BP.Services
{
    public class PasswordService
    {
        ...
        public PasswordService(BusinessSettings businessSettings)
        {
        ...
        public bool PasswordCheck(string password, string dbSalt, byte[] dbHash)
        {
            byte[] comparedHash = Hash(password, dbSalt);
            if (comparedHash == dbHash)
            {
                return true;
            }
            else
